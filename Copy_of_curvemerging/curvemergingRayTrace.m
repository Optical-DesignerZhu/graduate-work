function [A ]= curvemergingRayTrace( W , rangeX , rangeY, piexN, h, L)
%   W为zernike前15项中每一项所占的比重  
%   rangeX和rangeY分别是结构光投射在X轴和Y轴的取值范围
%  N代表像素点分割的个数
%  Z代表CCD距离表面的高度
%  L代表CCD的半长度
...
sx =linspace(rangeX(2),rangeX(1),201);                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    %光源X轴的取值范围
sy =linspace(rangeY(2),rangeY(1),201);                                       %光源Y轴的取值范围
[X,Y] = meshgrid(sx, sy);
X = X(:)';
Y = Y(:)';
% sx =[X; X];
...
for i = 1:piexN
    for j =1:piexN
     A(i,j) = length(find(prx<=-L - 0.1 + rangeX(1) + 2*L*i/piexN ...%选择CCD的范围为-2到2
            & prx> -L - 0.1 + rangeX(1) + (2*L*i - 2*L)/piexN & pry<=-L - 0.1 + rangeY(1) + 2*L*j/piexN & pry>-L - 0.1 + rangeY(1) + (2*L*j - 2*L)/piexN));
    end
end
end
